...
class TextClassifier:
    def __init__(self, model_dir='models_new'):
        """
        Initialize the text classifier with pre-trained models_new.
        
        Args:
            model_dir (str): Directory containing saved models_new and configurations.
        """
        logger.info("Initializing TextClassifier...")
        self.model_dir = model_dir

        tokenizer_sen_path = 'models_new/ta_sentence_2/tokenizer'
        semantic_sen_model = 'models_new/ta_sentence_2/semantic_model.h5'

        tokenizer_par_path = 'models_new/ta_paragraph_2/tokenizer'
        semantic_par_model = 'models_new/ta_paragraph_2/semantic_model.h5'
        ...
        try:
            ...
            required_files = [
                ...
                tokenizer_par_path,
                semantic_par_model,
                f'{model_dir}/ta_sentence_2/classification_model.h5',
                f'{model_dir}/ta_paragraph_2/classification_model.h5',
                f'{model_dir}/ta_sentence_2/scaler_linguistic.pkl',
                f'{model_dir}/ta_paragraph_2/scaler_linguistic.pkl',
                f'{model_dir}/ta_sentence_2/tokenized_data.pkl',
                f'{model_dir}/ta_paragraph_2/tokenized_data.pkl',
                f'{model_dir}/ta_sentence_2/reference_embeddings_new.pkl',
                f'{model_dir}/ta_paragraph_2/reference_embeddings_new.pkl',
            ]
            ...
            with tf.keras.utils.custom_object_scope(custom_objects):
                self.semantic_sen_model = tf.keras.models.load_model(
                    semantic_sen_model
                )
                logger.info("Semantic sentence model loaded")
                
                self.semantic_par_model = tf.keras.models.load_model(
                    semantic_par_model
                )
                logger.info("Semantic paragraph model loaded")
                
                self.classifier_sen_model = tf.keras.models.load_model(
                    f'{model_dir}/ta_sentence_2/classification_model.h5'
                )
                logger.info("Classification sentence model loaded")
                
                self.classifier_par_model = tf.keras.models.load_model(
                    f'{model_dir}/ta_paragraph_2/classification_model.h5'
                )
                logger.info("Classification paragraph model loaded")

            # Load scaler for linguistic features
            logger.info("Loading scalers and tokenized data...")
            with open(f'{model_dir}/ta_sentence_2/scaler_linguistic.pkl', "rb") as f:
                self.scaler_sen = pickle.load(f)

            with open(f'{model_dir}/ta_paragraph_2/scaler_linguistic.pkl', "rb") as f:
                self.scaler_par = pickle.load(f)

            with open(f'{model_dir}/ta_sentence_2/reference_embeddings_new.pkl', 'rb') as f:
                self.sentence_embeddings = pickle.load(f)

            with open(f'{model_dir}/ta_paragraph_2/reference_embeddings_new.pkl', 'rb') as f:
    ...
    def classify_sentence(self, sentence):
        ...
        try:
            ...
            student_similarity = self.cos_sim(embedding, self.sentence_embeddings['embeddings_std_sen'])
            chatgpt_similarity = self.cos_sim(embedding, self.sentence_embeddings['embeddings_gpt1_sen'])
            chatgpt_knowledge_similarity = self.cos_sim(embedding, self.sentence_embeddings['embeddings_gpt2_sen'])
            ...
            class_label = "AI" if prediction > 0.4095 else "Human"
    ...
    def classify_paragraph(self, paragraph):
        ...
        try:
            ...
            student_similarity = self.cos_sim(embedding, self.paragraph_embeddings['embeddings_std_par'])
            chatgpt_similarity = self.cos_sim(embedding, self.paragraph_embeddings['embeddings_gpt1_par'])
            chatgpt_knowledge_similarity = self.cos_sim(embedding, self.paragraph_embeddings['embeddings_gpt2_par'])
            ...
            class_label = "AI" if prediction > 0.5371 else "Human"
...
class CachedTextClassifier(TextClassifier):
    def __init__(self, model_dir='models_new', cache_size=128):
